/******************************************************************************
 *    Nome:                   Davit
 *    Copyright/ProprietÃ¡rio: 2010/ Renato da Silva Louro (@rslouro)
 *    Projeto:                Davit     
 *    Gestor do Arquivo:      Renato da Silva Louro (@rslouro) 
 *                               renato@silostecnologia.com.br
 *    Arquivo:                main.js
 *    IdentificaÃ§Ã£o:          dvt
 *    VersÃ£o corrente:        00.001 Alfa
 *    Data de AprovaÃ§Ã£o:      2010/12/10
 *    LicenÃ§a: GNU General    Public License, version 3 (GPLv3)
...
{
    makeworld(0,0,false); 
    document.getElementById('level').options.selectedIndex = 0;
    document.getElementById('programaDavit').value = "asdasds";
}

function setupFacil()
{
   makeworld(14,14,true);
createLongWall([3,0], 'L',4);
createLongWall([3,4], 'N',8);
createDisc([5,2],"rgb(0,0,255)");
createDisc([3,2],"rgb(0,0,255)");
createDisc([12,5],"rgb(0,0,255)");
}

function setupMedio()
{
    makeworld(14,14,true);
createLongWall([1,0], 'L',10);
createLongWall([0,10], 'N',10);
createLongWall([5,6], 'L',6);
createDisc([6,10],"rgb(0,0,255)");
createDisc([5,11],"rgb(0,0,255)");
createDisc([3,2],"rgb(0,0,255)");
createDisc([13,0],"rgb(0,0,255)");
}

function setupDificil()
{
    /* Construa o seu cenário abaixo ou carreque um já gravado. */

makeworld(14,14,true);

createDisc([3,5],"rgb(0,0,255)");
createDisc([10,1],"rgb(0,0,255)");
createDisc([11,11],"rgb(0,0,255)");
createDisc([1,10],"rgb(0,0,255)");
createDisc([1,9],"rgb(0,0,255)");
createDisc([4,11],"rgb(0,0,255)");
createDisc([7,11],"rgb(0,0,255)");
createDisc([4,4],"rgb(0,0,255)");

createDavit(1,0,'N');
createLongWall([6,1], 'N',2);
createLongWall([5,2], 'L',5);
createLongWall([1,6], 'N',5);
createLongWall([2,4], 'N',3);
createLongWall([1,7], 'N',7);
createLongWall([9,1], 'N',2);
createLongWall([0,8], 'L',4);
createLongWall([7,9], 'L',3);
createLongWall([3,9], 'L',3);
createLongWall([5,9], 'L',3);
createLongWall([1,9], 'N',2);
createLongWall([1,11], 'N',11);
createLongWall([9,10], 'N',3);
createLongWall([11,11], 'L',1);

createLongWall([1,0], 'L',3);
createLongWall([0,0], 'L',7);
createLongWall([2,2], 'N',3);
createLongWall([4,1], 'L',2);
createLongWall([5,0], 'N',6);
createLongWall([9,1], 'N',2);
createLongWall([8,2], 'L',9);
createLongWall([7,2], 'L',6);
createWall([10,1], 'L');
...
function createDavit(piPosC, piPosL, psDir, pqntDisc, psDefaultDiscColor)
{
    ...
    iPosL = piPosL;
    G_sDir = psDir;
    if (pqntDisc != undefined)
        G_qntDisc = pqntDisc;
    if (psDefaultDiscColor != undefined)
        G_sDefaultDiscColor = psDefaultDiscColor;
    redrawAll();
}
...
{
    if (G_arDisc[getIdDisc(parPoint)] == null)
        G_arDisc[getIdDisc(parPoint)] = new Array();
...
function createLongWall(parPoint, psDir, piLength)
{
    ...
    {
        if (psDir == 'L' || psDir == 'O')
            createWall([parPoint[0], parPoint[1] + i], psDir);
        if (psDir == 'N' || psDir == 'S')
            createWall([parPoint[0] + i, parPoint[1]], psDir);
    }
...
function putDisc()
{
    if (G_arrDavitDisc.length > 0)
    {
        ...
        if (G_arDisc[getIdDisc([iPosC, iPosL])] == undefined)
            G_arDisc[getIdDisc([iPosC, iPosL])] = [];
